...
<table class="selector row-highlight">
  ...
  {/if}
    ...
    {* remove action-item for popup edit *}
    {*before: <td>{$row.action|replace:'xx':$row.grant_id}</td> *}
    <td>{$row.action|replace:'xx':$row.grant_id|replace:'action-item ':''|replace:'small-popup':'action-item small-popup'}</td>
   </tr>
  {/foreach}
